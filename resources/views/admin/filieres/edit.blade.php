...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-black text-slate-900 tracking-tight">Modifier la Filière : {{ $filiere->name }}</h1>
        ...
        <form action="{{ route('admin.filieres.update', $filiere) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4"
                    class="input">{{ old('description', $filiere->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
